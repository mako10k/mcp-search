...
export class CacheManager {
    ...
    store(query: string, googleResults: any[]): SearchResponse {
    ...
    cleanup(): void {
        ...
        for (const [searchId, cache] of this.cache.entries()) {
            if (this.isExpired(cache)) {
                this.removeCache(searchId);
            }
        }
    ...
    listSearchHistory(options?: {
        ...
        logger.info(`Retrieved search history: ${searches.length}/${totalCount} entries (page ${page}/${totalPages})`);
