...
import type { ListSearchCacheParams } from './types';
...
export async function listSearchCache(params: ListSearchCacheParams) {
  try {
    ...
    logger.info(
      `Retrieving search cache list: keyword="${keyword || 'none'}", page=${page}, limit=${limit}`,
    );

    return cacheManager.listSearchHistory({
      keyword,
      page,
      limit,
    });
