...
const SearchParamsSchema = z.object({
  ...
  language: z
    ...
    .optional()
    .describe(
      'The language for the search results (ISO 639-1 codes). Should only be used if explicitly requested by the user or deemed necessary by the AI model.',
    ),
  region: z
    .enum(['US', 'JP', 'ES', 'FR', 'DE', 'CN', 'RU', 'AR', 'BR', 'IT'])
    .optional()
    .describe(
      'The region for the search results (ISO 3166-1 alpha-2 codes). Should only be used for region-specific software development or when explicitly requested by the user.',
    ),
  numResults: z.number().optional().describe('The number of search results to return (1..10).'),
  startIndex: z
    .number()
    .optional()
...
export async function searchEngine(
  ...
  try {
    ...
    if (err.response) {
      ...
      const apiMsg = extractApiErrorMessage(err.response.data);
      return {
        error: true,
        status: err.response.status,
        message: apiMsg,
    ...
    logger.error('Network or server error:', msg);
    return {
      error: true,
