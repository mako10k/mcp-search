...
class FetchCacheManager {
  ...
  constructor() {
    ...
    logger.info(
      `FetchCacheManager initialized with limits: MAX_FILE_SIZE: ${MAX_FILE_SIZE} bytes (${(MAX_FILE_SIZE / 1024 / 1024).toFixed(1)}MB), MAX_TOTAL_CACHE: ${MAX_TOTAL_CACHE_SIZE} bytes (${(MAX_TOTAL_CACHE_SIZE / 1024 / 1024).toFixed(1)}MB)`,
    );
  }
    ...
    try {
      ...
      const errObj = error as
        | {
            ...
            code?: string;
            errno?: string;
            cause?: { code?: string; errno?: string; message?: string };
          }
        | undefined;
      ...
      } else if (errObj?.cause?.message) {
        errMsg = errObj.cause.message as string;
      } else {
      ...
      errCode =
        errCode || errObj?.code || errObj?.errno || errObj?.cause?.code || errObj?.cause?.errno;
  ...
  listFetchHistory(
    ...
    for (const [id, cache] of this.cache.entries()) {
      if (!this.isExpired(cache) && (!requestId || id === requestId)) {
        history.push({
  ...
  getProcessedView(requestId: string, options?: ProcessViewOptions): FetchResult | null {
    ...
    const rawPreview = includeRawPreview
      ? cache.data.slice(0, rawPreviewSize).toString('utf-8')
      : undefined;
  ...
  private enforceStorageLimit(newDataSize: number): void {
    ...
    if (projectedTotal > MAX_TOTAL_CACHE_SIZE) {
      logger.info(
        `Total cache size would exceed limit: ${projectedTotal} > ${MAX_TOTAL_CACHE_SIZE}. Evicting old entries.`,
      );
      ...
      while (freedSize < excessSize && this.accessOrder.length > 0) {
        ...
        if (oldestRequestId && this.cache.has(oldestRequestId)) {
          ...
          logger.info(
            `Evicted cache entry ${oldestRequestId} (${oldCache.data.length} bytes) for storage limit`,
          );
        }
      }
    }
